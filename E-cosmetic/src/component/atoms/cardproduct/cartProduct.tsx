...
const CardProduct: React.FC<CardProductProps> = ({
  ...
  const renderCols = () => {
    if (isLoading) {
      return (
        <div style={{ marginLeft: '50%', marginTop: '10%' }}>
          {' '}
          <Spin style={{ color: '#7eaa92' }} className='custom-spin' tip='Loading' size='large'>
    ...
    const cols = products.slice(startIndex, endIndex).map((product) => (
      <Col className='col' sm={12} md={12} lg={8} xl={6} key={product.id}>
        <Space direction='vertical' size='large' style={{ display: 'flex' }}>
          <Link
            to={`/products/${product.product_name}/${product.id}`}
